...
| [实验设计](validate/experiment-design.md) | Validate 阶段用户考虑"先做个东西"来验证想法时 | 用最小投入设计验证实验，把假设写成可测试的预测，根据结果决定继续/转向/放弃 |
...
| [API 设计哲学](plan/api-design.md) | Plan 阶段进入"架构"章节，开始设计系统 API 接口 | 先设计 API 再实现，用一致性、直觉性、版本化和文档化构建调用者友好的接口 |
| [数据建模](plan/data-modeling.md) | Plan 阶段进入"架构"章节，开始设计数据库 schema 或数据模型 | 从业务概念出发设计数据模型，用约束保障数据完整性，避免过度设计和命名不一致 |
| [安全设计](plan/security-design.md) | Plan 阶段进入"架构"章节，系统涉及用户数据或外部访问 | 在设计阶段融入安全考量，用最小权限、输入校验、安全库和响应计划构建安全体系 |
| [RFC / 设计文档](plan/rfc-design.md) | 项目进入多人协作阶段，或用户说"我们讨论过但大家理解不一样" | 用书面化决策替代口头决策，让团队对齐、让历史可查 |
...
| [可观测性](track/observability.md) | Track 阶段系统已上线，但无法判断系统运行状态 | 用日志/指标/追踪三支柱建立主动监控，在问题发生前发现异常，而非等用户投诉 |
...
| [技术债管理](track/tech-debt.md) | 用户说"代码越来越难改了"或"先快速实现，后面再重构" | 区分有意债务和糟糕代码，用 20% 还债时间和童子军规则控制技术债累积 |
...
| [功能蔓延——如何避免功能越加越多](cross/feature-creep.md) | 规划阶段功能列表不断增长，或用户说"这个功能加上去应该会有人用" | 全阶段 | 控制功能增长，避免产品臃肿、维护成本飙升、核心价值模糊 |
...
| [增长飞轮](cross/growth-flywheel.md) | 产品上线后需要增长，或用户说"产品做好了但没人用" | Cross, Track | 找到增长循环让每个用户带来更多用户，而非依赖外部流量或加功能 |
| [用户留存](cross/user-retention.md) | 产品上线后用户流失率高，或用户说"用户来了但留不住" | Cross, Track | 理解用户为什么留下和离开，通过激活、留存曲线和关键行为分析优化留存 |
| [产品战略](cross/product-strategy.md) | 产品有多个方向可选，或用户说"不知道优先做什么" | Cross, Plan | 定义"我们是谁、为谁服务、解决什么问题"，用战略指导每个功能决策 |
| [持续发现](cross/continuous-discovery.md) | 产品上线后认为"已经验证过了，不需要再验证了" | Cross, Validate, Track | 在产品生命周期中持续了解用户需求，让发现结果驱动路线图 |
...
| 当前阶段 | 自动加载的卡片 | 匹配逻辑 |
|----------|--------------|---------|
| **Discover** | `discover/mom-test`, `discover/scratch-your-itch`, `discover/jtbd-work-statement`, `discover/secret-test`, `discover/dirty-work-test`, `cross/beachhead`, `cross/say-no-by-default`, `cross/complexity-budget`, `cross/handmade-validation`, `cross/pmf-survey`, `cross/user-behavior-metric`, `cross/async-communication`, `cross/premature-optimization`, `cross/feature-creep`, `cross/analysis-paralysis`, `cross/unicorn-mindset` | 所有 Discover 阶段卡 + 跨阶段通用卡 + 反模式系列 |
| **Validate** | `validate/test-card`, `validate/handmade-first`, `validate/behavior-signal`, `validate/continuous-check`, `validate/pricing-test`, `validate/competitive-analysis`, `validate/experiment-design`, `validate/gonogo-decision`, `validate/user-interview`, `cross/say-no-by-default`, `cross/complexity-budget`, `cross/handmade-validation`, `cross/pmf-survey`, `cross/user-behavior-metric`, `cross/async-communication`, `cross/continuous-discovery`, `cross/premature-optimization`, `cross/feature-creep`, `cross/analysis-paralysis`, `cross/unicorn-mindset` | 所有 Validate 阶段卡 + 跨阶段通用卡 + 反模式系列 |
| **Plan** | `plan/strategy-kernel`, `plan/appetite-constraint`, `plan/c4-architecture`, `plan/bounded-context`, `plan/adr-rfc-enhance`, `plan/pitch-format`, `plan/stability-patterns`, `plan/test-first`, `plan/document-as-deliverable`, `plan/api-design`, `plan/data-modeling`, `plan/security-design`, `plan/rfc-design`, `cross/beachhead`, `cross/say-no-by-default`, `cross/complexity-budget`, `cross/handmade-validation`, `cross/pmf-survey`, `cross/user-behavior-metric`, `cross/async-communication`, `cross/product-strategy`, `cross/premature-optimization`, `cross/feature-creep`, `cross/analysis-paralysis`, `cross/unicorn-mindset` | 所有 Plan 阶段卡 + 跨阶段通用卡 + 反模式系列 |
| **Track** | `track/certainty-level`, `track/wip-detection`, `track/constraint-diagnosis`, `track/four-metrics`, `track/milestone-state`, `track/heartbeat`, `track/integration-health`, `track/regular-departure`, `track/value-stream`, `track/observability`, `track/code-review`, `track/retrospective`, `track/tech-debt`, `cross/beachhead`, `cross/say-no-by-default`, `cross/complexity-budget`, `cross/handmade-validation`, `cross/pmf-survey`, `cross/user-behavior-metric`, `cross/growth-flywheel`, `cross/user-retention`, `cross/async-communication`, `cross/continuous-discovery`, `cross/premature-optimization`, `cross/feature-creep`, `cross/analysis-paralysis`, `cross/unicorn-mindset` | 所有 Track 阶段卡 + 跨阶段通用卡 + 反模式系列 |
...
- 用户说"我先做个 MVP"或"先写代码"来验证想法 → 加载 `experiment-design`
...
- 用户说"先做出来试试"而验证信号不明确 → 加载 `gonogo-decision`
...
- 用户说"再做个功能，用户就会来了" → 加载 `growth-flywheel`
- 用户说"我们有很多用户，但增长停了" → 加载 `growth-flywheel`
...
- 规划阶段功能列表不断增长，但从未删减过 → 加载 `feature-creep`
- 用户说"这个功能加上去应该会有人用" → 加载 `feature-creep`
