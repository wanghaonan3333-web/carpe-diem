...
class SkillStructureTests(unittest.TestCase):
    ...
    def test_skill_discovery_and_output_language_are_bilingual(self):
        text = (ROOT / "SKILL.md").read_text(encoding="utf-8")
        frontmatter = text.split("---", 2)[1]

        self.assertIn("project direction", frontmatter)
        self.assertIn("项目方向", frontmatter)
        self.assertIn("匹配用户当前使用的语言", text)
