...
这是 Carpe Diem 的 `v0.1` 初版。我们仍在验证它是否真的能帮助人从“拥有 Agent”走到“开始一个值得做的项目”。非常欢迎你试用后提交积极、具体且诚实的反馈——尤其是哪里真正帮到了你、哪里让你困惑，以及它在哪一轮失去了主线。每一个真实案例都会帮助下一版变得更好。
...
四个平台共用同一份 Skill 方法和状态协议。v0.1 已自动验证 Skill 结构、确定性脚本和四种安装目标，并在 Codex CLI `0.150.0-alpha.8` 完成用户级自动发现与三轮隔离行为演练。Claude Code、Cursor、OpenClaw 目前仍是“官方目录与安装快照已验证，真实宿主触发待测”。仓库内的[内测记录](docs/internal-testing.md)会持续区分“已验证”和“待验证”，不会把可行推断写成已支持事实。
