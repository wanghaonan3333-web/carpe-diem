...
Carpe Diem 目前仍处于早期公开版本。非常欢迎你试用后提交具体、诚实的反馈——尤其是哪里真正帮到了你、哪里让你困惑，以及它在哪一轮失去了主线。

当前版本：`v0.3.0`。智慧库现有 55 张行为卡。运行时把阶段作为候选过滤器，只在出现具体信号时加载一张主卡和至多一张辅卡；贡献流程区分“研究层可追溯”和“运行时不露来源”，并加入证据核验、去重、场景测试与单写者集成门槛。

## v0.3.0 更新了什么

- 新增 22 张覆盖验证、协作、产品、工程和反模式的证据支持行为卡。
- 将整阶段批量读取改为由具体用户信号驱动的“一张主卡 + 至多一张辅卡”路由。
- 新增证据登记、重叠检查、场景验收和单写者集成规范，方便继续蒸馏新智慧。
- 加固本地安装指导，移除安装器中的远程管道执行。
...
四个平台共用同一份 Skill 方法和状态协议。v0.3.0 已自动验证 55 张行为卡、确定性脚本、路由场景和四种安装目标，并在 Codex CLI `0.150.0-alpha.8` 完成用户级自动发现与三轮隔离行为演练。Claude Code、Cursor、OpenClaw 目前仍是“官方目录与安装快照已验证，真实宿主触发待测”。仓库内的[内测记录](docs/internal-testing.md)会持续区分“已验证”和“待验证”，不会把可行推断写成已支持事实。
...
- 新增 22 张覆盖验证、协作、产品、工程和反模式的候选行为卡。
- 将整阶段批量读取改为两张卡的上下文预算和明确触发路由。
- 新增研究层证据登记、重叠门槛、场景验收和单写者集成规范。
- 修正定价、Go/No-Go、API、安全、代码审查、可观测性、技术债与异步沟通中的无依据通用阈值和高风险表述。

### v0.2.0 — 2026-08-30

**智慧蒸馏系统完成——33 张行为卡覆盖全阶段**

本次重大更新完成了智慧蒸馏工作，将 24 位思想领袖和 16 个工程案例的可操作洞察嵌入 Carpe Diem 的四阶段流程。

**对用户的新体验：**
- Carpe Diem 现在会根据你所在的阶段和具体信号，按需选择行为卡——比如你说"很多人说这个方向好"，它会用 Mom Test 的方法引导你区分真实信号和礼貌性赞美，整个过程你不需要知道卡片的存在
- 四个阶段全覆盖：Discover（5 张）、Validate（4 张）、Plan（9 张）、Track（9 张），外加 6 张跨阶段通用卡
- 每张卡提供 5-6 条具体、可操作步骤，可在 3-5 轮对话内执行

**对贡献者：**
- 完整的蒸馏流程（研究 → 规划 → 实现 → 集成）已记录在 `docs/wisdom-distillation-guide.md`
- 三条铁律：不露来源、不抽象、可操作
- `manifest.json` 已包含所有智慧卡，`bash install.sh` 会正确部署

### v0.1.1 — 2026-08-31

加固安装器路径校验，补齐生命周期恢复规则，限定会话授权复用范围，并改善中英双语发现与回复。
